...
int hal_camera_capture_one(hal_camera_frame_t *frame) {
    ...
    buf.type   = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory = V4L2_MEMORY_MMAP;
    if (ioctl(g_fd, VIDIOC_DQBUF, &buf) < 0) { perror("[CAM] DQBUF"); goto out_streamoff; }
    ...
    buf.memory = V4L2_MEMORY_MMAP;
    buf.index  = 0;
    if (ioctl(g_fd, VIDIOC_QUERYBUF, &buf) < 0) { perror("[CAM] QUERYBUF result"); goto out_streamoff; }

    frame->index  = 0;
    frame->length = buf.bytesused;
    frame->data   = mmap(NULL, buf.length, PROT_READ|PROT_WRITE, MAP_SHARED, g_fd, buf.m.offset);
    if (frame->data == MAP_FAILED) { perror("[CAM] mmap"); goto out_streamoff; }
    frame->buf    = buf;
    ret = 0;
...
int capture_take_photo(capture_photo_cb_t cb, void *user_data) {
    ...
    if (was_streaming)
        capture_start_preview(g_preview_cb, g_preview_ctx);
